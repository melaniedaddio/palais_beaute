...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.WARNING('[!] ATTENTION : Cette operation va supprimer toutes les donnees de test !'))
        self.stdout.write('')
        ...
        self.stdout.write('')
        self.stdout.write(self.style.WARNING('Les données de configuration (instituts, employés, prestations, produits, clients) seront CONSERV?ES.'))
        self.stdout.write('')

        if not options['confirm']:
            confirmation = input('Voulez-vous vraiment continuer ? Tapez "OUI" en majuscules pour confirmer : ')
            if confirmation != 'OUI':
                self.stdout.write(self.style.ERROR('[ERREUR] Opération annulée'))
                return
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(f'[OK] {count_rdv_options} options de RDV supprimées')

                # 8. Supprimer les rendez-vous
                count_rdv = RendezVous.objects.count()
                RendezVous.objects.all().delete()
                self.stdout.write(f'[OK] {count_rdv} rendez-vous supprimés')
                ...
                self.stdout.write(f'[OK] {count_mvt} mouvements de stock supprimés')
                count_produits = Produit.objects.update(stock_actuel=0)
                self.stdout.write(f'[OK] {count_produits} produits remis à stock_actuel=0')

                # 15. Inventaires (lignes avant entêtes)
                count_lignes_inv = LigneInventaire.objects.count()
                LigneInventaire.objects.all().delete()
                self.stdout.write(f'[OK] {count_lignes_inv} lignes d\'inventaire supprimées')

                count_inv = Inventaire.objects.count()
                Inventaire.objects.all().delete()
                self.stdout.write(f'[OK] {count_inv} inventaires supprimés')
                ...
                self.stdout.write(f'[OK] {count_valid_dep} validations de dépenses récurrentes supprimées')

                count_dep = Depense.objects.count()
                Depense.objects.all().delete()
                self.stdout.write(f'[OK] {count_dep} dépenses supprimées')

                # 17. Réconciliations de caisse
                count_reco = ReconciliationCaisse.objects.count()
                ReconciliationCaisse.objects.all().delete()
                self.stdout.write(f'[OK] {count_reco} réconciliations de caisse supprimées')

                self.stdout.write('')
                self.stdout.write(self.style.SUCCESS('[OK] Nettoyage terminé avec succès !'))
                self.stdout.write('')
                self.stdout.write('La base de données est maintenant propre et prête pour la production.')

        except Exception as e:
            self.stdout.write('')
            self.stdout.write(self.style.ERROR(f'[ERREUR] Erreur lors du nettoyage : {str(e)}'))
